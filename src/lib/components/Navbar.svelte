<script>
    ...
    const themeState = themeCtx || { isDark: true, toggle: () => {} };
    ...
    let navItems = [
        ...
        { label: 'Mechanism', href: '/mechanism', isButton: false },
    ...
    <div class="mx-auto flex max-w-6xl items-center justify-between">
        ...
        <div class="flex items-center gap-2 font-bold text-xl">
            <div class="h-8 w-8 rounded-lg bg-rose-600 flex items-center justify-center text-white text-sm">A</div>
                {#if isHome}
                    <span>
                        Aiko <span class="text-rose-600">Playground</span>
                    </span>
                {:else}
                    <span>
                        Aiko <span class="text-rose-600">Docs</span>
                    </span>
                {/if}
        </div>
                ...
                {#each navItems as item}
                    {#if item.isButton}
                        {#if !isDocs}
                            <li>
                                <a href={item.href}
                                class="rounded-full bg-rose-600 px-5 py-2 text-sm font-bold text-white shadow-lg shadow-rose-500/30 transition-all hover:bg-rose-700 hover:scale-105 active:scale-95">
                                    {item.label}
                                </a>
                            </li>
                        {/if}
                    {:else}
                        <li>
                            <a href={item.href}
                            class="rounded-lg px-4 py-2 text-sm font-medium transition-colors hover:text-rose-600
    ...
    {#if isMenuOpen}
        <div class="absolute left-0 top-full w-full border-b p-6 shadow-xl md:hidden 
            ...
            <ul class="flex flex-col gap-4">
                {#each navItems as item}
                    {#if !item.isButton || (item.isButton && !isDocs)}
                        <li>
                            {#if item.isButton}
                                <a href={item.href} 
                                   onclick={() => isMenuOpen = false} 
                                   class="block w-full text-center rounded-xl bg-rose-600 px-5 py-3 text-sm font-bold text-white shadow-lg shadow-rose-500/30 transition-all hover:bg-rose-700 active:scale-95">
                                    {item.label}
                                </a>
                            {:else}
                                <a href={item.href} 
                                   onclick={() => isMenuOpen = false} 
                                   class="block text-lg font-medium {themeState.isDark ? 'text-zinc-300' : 'text-gray-600'}">
                                    {item.label}
                                </a>
                            {/if}
                        </li>
